...
function App() {
  ...
  useEffect(() => {
    fetch('http://127.0.0.1:5055/api/gifts')
      .then((response) => {
  ...
  async function handleRsvp(event) {
    ...
    try {
      const response = await fetch('http://127.0.0.1:5055/api/rsvps', {
        method: 'POST',
  ...
  async function reserveGift(event) {
    ...
    setGiftError('')
    try {
      const response = await fetch(`http://127.0.0.1:5055/api/gifts/${selectedGift.id}/reserve`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
